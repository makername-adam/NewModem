...
function [resultsFixed, resultsAdaptive] = sim_fixed_and_adaptive( ...
    ...
    for m = 1:length(modSchemes)
        ...
        SSIMVals = zeros(size(snrRange_dB));
        ...
        for si = 1:length(snrRange_dB)
                ...
                for b = 1:numBlocks
                    ...
                    while ~success && txCount < harqMaxTx
                        ...
                        if isempty(llrAccum)
                            ...
                            llrAccum(1:L) = llrAccum(1:L) + llr(1:L);
                        end
            ...
            SSIMVals(si) = SSIM_calc(img, imgRec);
            thr(si)      = (bitsPerFrame * tbSucc) / max(1, totalSubcUses);

            fprintf('%s: SNR=%2d dB, BER=%.3e, PSNR=%.2f, SSIM=%.3f, Thr=%.3f\n',...
                label, snr_dB, ber(si), psnrVals(si), SSIMVals(si), thr(si));
        end

        resultsFixed(m).label = label;
        resultsFixed(m).ber   = ber;
        resultsFixed(m).psnr  = psnrVals;
        resultsFixed(m).ssim  = SSIMVals;
    ...
    for si = 1:length(snrRange_dB)
        ...
        SSIMA (si) = SSIM_calc(img, imgRec);
        thrA(si)  = (bitsPerFrame * tbSucc) / max(1, totalSubcUses);

        fprintf('Adaptive: SNR=%2d dB, BER=%.3e, PSNR=%.2f, SSIM=%.3f, Thr=%.3f\n',...
            snr_dB, berA(si), psnrA(si), SSIMA(si), thrA(si));
    end

    resultsAdaptive.label = 'Adaptive';
    resultsAdaptive.ber   = berA;
    resultsAdaptive.psnr  = psnrA;
    resultsAdaptive.ssim = SSIMA;
...
function [score, ssimMap] = SSIM_calc(refImg, receivedImg)
    if any(size(refImg) ~= size(receivedImg))
        receivedImg = imresize(receivedImg, [size(refImg, 1), size(refImg, 2)]);
    end
    [score, ssimMap] = ssim(receivedImg, refImg);
end
...
function plot_results_5g_channel(resultsFixed, resultsAdaptive, snrRange_dB, chanName)
    ...
    title(sprintf('Throughput vs SNR (channel-use normalized) (%s)',chanName));
        ...
        % -------- SSIM vs SNR --------
    figure('Name',sprintf('SSIM vs SNR (%s)',chanName),'NumberTitle','off');
    hold on; grid on;
    for k = 1:length(resultsFixed)
        plot(snrRange_dB, resultsFixed(k).ssim, '-o','LineWidth',1.5);
    end
    plot(snrRange_dB, resultsAdaptive.ssim, '-s','LineWidth',1.8);
    xlabel('SNR (dB)'); ylabel('SSIM');
    legend(labels, 'Location','southeast');
    title(sprintf('SSIM vs SNR (channel-use normalized) (%s)',chanName));
end
